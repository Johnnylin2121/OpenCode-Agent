...
def main():
    ...
    if args.command == 'preprocess':
        ...
        # 确定输出路径
        if args.output:
            output_path = str(safe_output_path(args.output))
        else:
        ...
        if args.output:
            ...
            output_dir = safe_output_path(args.output_dir or default_output_dir())
            now = datetime.now().strftime('%Y-%m-%d')
            filename = Path(args.input).stem
            output_path = str(safe_output_path(Path(output_dir) / f"{now} {filename} 选品分析.md"))
        ...
        if args.output:
            ...
            now = datetime.now().strftime('%Y-%m-%d')
            filename = Path(args.input).stem
            output_path = str(safe_output_path(Path(output_dir) / f"{now} {filename} ABA选品分析.md"))
        ...
        if args.output:
            ...
            output_path = str(safe_output_path(Path(output_dir) / f"{now} 深度分析 {safe_keyword}.md"))
    ...
    # 6种选品筛选方法的独立命令
        ...
        if args.output:
            ...
            output_dir = safe_output_path(args.output_dir or default_output_dir())
            output_path = str(safe_output_path(Path(output_dir) / f"{now} {filename} {label}筛选.md"))
