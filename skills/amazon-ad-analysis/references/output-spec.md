...
| # | 交付物 | 路径 | 读者 |
|---|--------|------|------|
| 1 | Markdown 报告 | `{OPENCODE_OUTPUT_ROOT}/amazon/ad-analysis/{日期}/{日期} {店铺} 全店分析报告.md`<br>`{OPENCODE_OUTPUT_ROOT}/amazon/ad-analysis/{日期}/{ASIN} 分析报告.md` | 人（OpenCode 记录） |
| 2 | Excel 工作簿 | 全店：`{OPENCODE_OUTPUT_ROOT}/amazon/ad-analysis/attachments/{日期} {店铺} 分析数据.xlsx`<br>单品：`{OPENCODE_OUTPUT_ROOT}/amazon/ad-analysis/attachments/{ASIN} 分析数据.xlsx` | 人（筛选）+ 脚本（校验） |
| 3 | 阈值校准台账 | `{OPENCODE_OUTPUT_ROOT}/amazon/ad-analysis/calibration/阈值校准台账.md` | 阈值迭代 |
...
| 4.3 核心词自然位缺口 | SKILL Phase 5.4-B | **B**（需用户导出或公开替代数据） |
...
路径：`{OPENCODE_OUTPUT_ROOT}/amazon/ad-analysis/calibration/阈值校准台账.md`
写入时机：每次分析后写入 OpenCode 非 Vault 校准草稿。一行 = 一个 ASIN 一次分析。
