...
def main():
    ...
    if args.command == 'clean':
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        import yaml
        target.write_text(yaml.safe_dump(config_template, allow_unicode=True, default_flow_style=False), encoding='utf-8')
        print(f"配置文件模板已生成: {target}")
